...
let d_version2 ~num_glyphs ~remained_length : Value.Post.GlyphNameArray.t decoder =
  let open DecodeOperation in
...
let get (src : source) : Value.Post.t ok =
  ...
  let dec =
    ...
    d_uint16 >>= fun underline_position ->
    d_uint16 >>= fun underline_thickness ->
    ...
    begin
      if is_long then
        d_version2 ~num_glyphs:common.num_glyphs ~remained_length:(length - 32) >>= fun names ->
        return @@ Some(names)
      else
        return None
    end >>= fun names_opt ->
